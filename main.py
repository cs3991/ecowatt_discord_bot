...
async def send_message():
    if datetime.datetime.now().hour == 8:
        print('8:00 : leaderboard sent !')
        channel = bot.get_channel()
        await channel.send(
            f'Jour {datetime.date.today().day} de Advent of Code : https://adventofcode.com/2021/day/{datetime.date.today().day}')
        await channel.send('Leaderboard : ')
        await leaderboard(None, channel=channel)
...
async def leaderboard(context, num_players: int = 20, channel=None):
    ...
    for i, player in enumerate(players):
        leaderboard.append(PLAYER_STR_FORMAT.format(rank=i + 1, name=player[0], name_pad=max_name_len, points=player[1],
                                                    points_pad=max_points_len, stars=player[2], stars_pad=max_stars_len,
                                                    star_time=time.strftime('%H:%M %d/%m', time.localtime(player[3]))))

    if context is None:
        await output_leaderboard(channel, leaderboard)
    else:
...
async def rank(context, *name):
    ...
    if players:
        ...
        i, player = players[0]
        result = '```'
        result += PLAYER_STR_FORMAT.format(rank=i + 1, name=player[0], name_pad=len(player[0]), points=player[1],
                                           points_pad=len(str(player[1])), stars=player[2],
                                           stars_pad=len(str(player[2])),
                                           star_time=time.strftime('%H:%M %d/%m', time.localtime(player[3])))
        result += '```'
    else:
...
async def keen(context):
    ...
    result += PLAYER_STR_FORMAT.format(rank=i + 1, name=player[0], name_pad=len(player[0]), points=player[1],
                                       points_pad=len(str(player[1])), stars=player[2], stars_pad=len(str(player[2])),
                                       star_time=time.strftime('%H:%M %d/%m', time.localtime(player[3])))
    result += '```'
    await context.send(result)
...
async def daily(context, day: str = None):
    ...
    if not final_table:
        ...
        for place, player in enumerate(final_table):
            leaderboard.append(
                PLAYER_STR_FORMAT.format(rank=place + 1, name=player[0], name_pad=max_name_len, points=player[1],
                                         points_pad=max_points_len, stars=player[3], stars_pad=max_stars_len,
